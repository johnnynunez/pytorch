...
class CppCodeCache:
    ...
    def load_async(
        ...
        # init build option and command line for the main and optimized code
        main_build_option = None
        optimized_build_option = None
        main_cmd_line = None
        optimized_cmd_line = None
        main_path = None
        optimized_path = None
        # init build option and command line for the single compilation path
        input_path = None
        cpp_build_option = None
        vec_isa_cmd = None

        if config.cpp_wrapper_build_separate:
            # split compilation path - separate main code from optimized code
            ...
            # Single compilation path - combine main and optimized code
            cpp_build_option = CppTorchDeviceOptions(**compile_command)
            command_gen = CppBuilder(
                name="o", sources="i", BuildOption=cpp_build_option
            )
            # write function will calc source_code hash, the same source code with different
            # ISA level should be generate different hash.
            # So we need get a command_line which contains isa related parameter as a part of hash key.
            ...
            key, input_path = write(
                (optimized_code or "") + main_code, "cpp", extra=vec_isa_cmd
            )

        # Return early if we already have this in the cache
        if key in cls.cache:
        ...
        if config.cpp_wrapper_build_separate:
            ...
            assert main_build_option is not None
            ...
            if config.cpp_cache_precompile_headers and not _IS_WINDOWS:
                ...
                if optimized_code and (header := _get_cpp_prefix_header(device_type)):
                    assert optimized_build_option is not None
            ...
            assert cpp_build_option is not None
            assert vec_isa_cmd is not None
...
class CppPythonBindingsCodeCache(CppCodeCache):
    ...
    def load_pybinding_async(
        ...
        Note: main_code and kernel_code are compiled separately only when cpp_wrapper_build_separate is True
        """
        parseargs = ", ".join(
