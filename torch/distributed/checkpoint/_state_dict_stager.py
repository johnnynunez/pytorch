...
class StateDictStager:
    ...
    def _stage_untyped_storage(
        ...
        if storage in self._cached_storage_mapping:
            ...
            assert cached_storage.size() == storage.size(), (
                "For async checkpointing,  We cache storages in DRAM and reuse them. Cached storage size does not match original storage size. This should never happen as we track the original storage weakref and clean up the cache storage. Please report this to PyTorch Distributed Checkpointing."
            )
    ...
    @torch.no_grad()
    def deepcopy_with_tensor_offload(self, x, memo=None, _nil=[], non_blocking=False):
